...
def extract_training_args(input_args=None):
    parser = argparse.ArgumentParser()
    parser.add_argument("--training_args_json", type=str, default=None, required=False)
    parser.add_argument(
        "--push_to_hub",
        action="store_true",
        help="Whether or not to push the model to the Hub.",
    ...
    if args.training_args_json:
        # json_data = json.loads(args.training_args_json)
    ...
    # move from parse_args over, since not using it
    log_queue, _ = start_queue(args.channel_log)
    write_log(log_queue)
...
def main(args):
    ...
    unet_lora_config = get_lora_config(
        rank=args.rank, use_dora=args.use_dora, target_modules=unet_target_modules
    )
    ...
    if args.train_text_encoder:
        ...
        text_lora_config = get_lora_config(
            rank=args.rank, use_dora=args.use_dora, target_modules=text_target_modules
        )
